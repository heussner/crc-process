...
tCK = float(thresholds[markers.index('CK')])
...
if 's1' in args.table:
    ...
    #sort by CK PPR
    sorted_table = table.sort_values(by=['cell_CK_PPR'],ascending=False,ignore_index=True).copy()
    
    #remove cells below CK threshold
    CK_positives = sorted_table[sorted_table['cell_CK_PPR'] >= tCK].copy()
    ...
    Negative_controls = table[table['cell_CK_PPR_raw'] == 0].copy()
