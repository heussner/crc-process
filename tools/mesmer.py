...
parser.add_argument('-r', '--refine-masks', default=True, type=bool)
...
app = Mesmer()
labeled_image = app.predict(im, compartment=args.compartment, image_mpp=args.mpp, batch_size=1)
...
if args.compartment == "both":
    if args.refine_masks:
        ...
        save_file = os.path.join(args.output, args.input.split('/')[-1] + "_NUC_unrefined__MESMER.tif")
        print("Saving cell output to {}".format(save_file))
        imwrite(save_file, labeled_image[:,:,0])
        save_file = os.path.join(args.output, args.input.split('/')[-1] + "_CELL_unrefined__MESMER.tif")
        print("Saving nuclei output to {}".format(save_file))
        imwrite(save_file, labeled_image[:,:,1])
